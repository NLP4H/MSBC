...
if __name__ == "__main__":
    prediction_col = "score_bowel_bladder_subscore"
    config = Params.from_file('master_path/master/ML4H_MSProject/task1/configs/cnn_encoder_score_bowel_bladder_subscore_config.jsonnet')
    serialization_dir = config["train_options"].pop("serialization_dir", None)
    params = config["params"]
    # Make predictions
    predictor = Predictor.from_path(archive_path=serialization_dir+"/model.tar.gz", cuda_device=0, predictor_name="predict.predictors.ms_note_predictor" ,dataset_reader_to_load=params["dataset_reader"])
