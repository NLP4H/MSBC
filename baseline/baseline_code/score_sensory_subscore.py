...
def check_vibration(note, limb_counts):
    score = -1
...
def check_temperature(note, limb_counts):
    score = -1
...
def check_proprioception(note, limb_counts):
    score = -1
...
def check_touch(note, limb_counts):
...
def check_pinprick(note, limb_counts):
    score = -1
...
def predict(edss, note):
    ...
    "unable to feel" in level_list:
        ...
        score = max(
            check_pinprick(selected_note,limb_counts),
            check_vibration(selected_note, limb_counts),
            check_temperature(selected_note,limb_counts),
            check_proprioception(selected_note,limb_counts),
            check_pinprick(selected_note,limb_counts),
...
if __name__ == "__main__":
    ...
    predictions = []
    labels = np.array(df["score_sensory_subscore"])
    notes = list(df["text"])

    for i in range(df.shape[0]):
        print(i) 
        predictions.append(score_sensory_prediction(i, df))
    ...
    for i in range(len(notes)):
        if labels[i] != 7 and predictions[i] != 7:
            total_count += 1
            if abs(labels[i] - predictions[i]) <= 1:
                count += 1
        if labels[i] == predictions[i]:
            count_1 += 1
    print("Accuracy: ", count_1 / len(labels))
    print("Converted Accuracy: ", count / total_count)
