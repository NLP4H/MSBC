...
def check_pupillary_response(note):
    ...
    return score
# TODO: For future use
def check_trigeminal_damage(note):
    
    score = -1
    print("Trigeminal damage: ", score)
    return score

def check_ptosis(note):
    # Unknown by default
    score = -1
...
def check_facial_weakness(note):
...
def predict(edss, note):
    ...
    if edss == 0:
        ...
        score = max(
            check_eom(selected_note),
            check_INO(selected_note), 
            check_nystagmus(selected_note),
            check_palate(selected_note),
            check_tongue(selected_note),
            check_pupillary_response(selected_note),
            check_trigeminal_damage(selected_note),
            check_ptosis(selected_note),
            check_facial_weakness(selected_note),
            check_hearing_loss(selected_note),
            check_dysarthria(selected_note),
            check_dysphagia(selected_note))
    return score
...
if __name__ == "__main__":
    ...
    predictions = []
    labels = np.array(df["score_brain_stem_subscore"])
    
    for i in range(df.shape[0]):
        print(i) 
        predictions.append(score_brain_stem_prediction(i, df))
        # Trouble Shooting
        # if predictions[i] != -1 and labels[i] != -1 and abs(predictions[i] - labels[i]) > 1:
        #     print(i)
        #    break

    
    # 
    # Transfer unknowns into another class
